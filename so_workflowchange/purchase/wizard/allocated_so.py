...
class allocated_so(osv.osv_memory):
    _name = 'allocated.so'
    _description = 'Allocated Sale Order'

    def default_get(self, cr, uid, fields, context=None):
        purchase_order_line_obj = self.pool.get('purchase.order.line')
        allocated_so_obj = self.pool.get("allocated.so")
        ...
        if context is None:
            context = {}
        res = super(allocated_so, self).default_get(cr, uid, fields, context=context)

        for lines in purchase_order_line_obj.browse(cr, uid, context.get(('active_ids'), []), context=context):
            sol_ids = []
            sale_order_line_ids = sale_order_line_obj.search(cr, uid, [('product_id','=',lines.product_id.id),('location_id','=',lines.location_dest_id.id),('state','<>','done'),('state','<>','draft'),('state','<>','cancel')])
            if sale_order_line_ids:
                for val in sale_order_line_ids:
                    sol = sale_order_line_obj.browse(cr, uid, val, context=context)
                    qtyp = product_uom_obj._compute_qty(cr, uid, sol.product_uom.id, sol.product_uom_qty, sol.product_id.uom_id.id)
                    spq = sol.product_id.spq
                    qty_allocated_onhand = sol.qty_onhand_allocated
                    sale_allocated_ids = sale_allocated_obj.browse(cr, uid, sale_allocated_obj.search(cr, uid, [('sale_line_id','=',sol.id)]), context=context)
                    qty_allocated = 0.00
                    if sale_allocated_ids:
                        for val2 in sale_allocated_ids:
                            qty_allocated = qty_allocated + val2.quantity
                    if qtyp > 0:
                        if qtyp - (qty_allocated + qty_allocated_onhand) > 0:
                            sol_ids.append(val)
            ...
            for o2 in sol_ids:
                sol2 = sale_order_line_obj.browse(cr, uid, o2, context=context)
                so_ids.append(sol2.order_id.id)

            so_ids = sorted(set(so_ids))

            if so_ids:
                sale_order_id = so_ids[0]
                solxx_ids = sale_order_line_obj.search(cr, uid, [('order_id','=',sale_order_id),('product_id','=',lines.product_id.id)])
                sale_order_line_id = solxx_ids[0]
            else:
                raise osv.except_osv(_('No Un-Allocated !'), _('no Un-Allocated found in Sale Order!'))

            sale_allocated_id = sale_allocated_obj.search(cr, uid, [('sale_line_id','=',sale_order_line_id),('purchase_line_id','=',lines.id)])
            ...
            qtyp3 = product_uom_obj._compute_qty(cr, uid, lines.product_uom.id, lines.product_qty, lines.product_id.uom_id.id)
            sale_allocated_ids3 = sale_allocated_obj.browse(cr, uid, sale_allocated_obj.search(cr, uid, [('purchase_line_id','=',lines.id),('receive','=',False)]), context=context)
            qty_allocated3 = 0.00
            qty_received3 = 0.00
            incoming_qty3 = 0.00
            ...
            if qtyp3 > 0:
                stock_move_ids3 = stock_move_obj.search(cr, uid, [('purchase_line_id','=',lines.id),('state','=','done')])
                if stock_move_ids3:
            ...
            sol4 = sale_order_line_obj.browse(cr, uid, sale_order_line_id, context=context)
            qtyp4 = product_uom_obj._compute_qty(cr, uid, sol4.product_uom.id, sol4.product_uom_qty, sol4.product_id.uom_id.id)
            qty_onhand_allocated4 = sol4.qty_onhand_allocated
            sale_allocated_ids4 = sale_allocated_obj.browse(cr, uid, sale_allocated_obj.search(cr, uid, [('sale_line_id','=',sale_order_line_id)]), context=context)
            ...
            if 'sale_order_line_id' in fields:
                res.update({'sale_order_line_id': sale_order_line_id or False})
            if 'purchase_order_line_id' in fields:
                res.update({'purchase_order_line_id': lines.id or False})
            ...
            if 'location_id' in fields:
                res.update({'location_id': lines.location_dest_id.id or False})
        return res
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        return super(allocated_so, self).write(cr, uid, ids, vals, context=context)

    def onchange_sale_order_id(self, cr, uid, ids ,sale_order_id, sale_order_line_id, product_id, location_id, context={}):
        ...
        sale_order_line_ids = sale_order_line_obj.search(cr, uid, [('product_id','=',product_id),('location_id','=',location_id),('state','<>','done'),('state','<>','draft'),('state','<>','cancel')])
        if sale_order_line_ids:
            for val in sale_order_line_ids:
                sol = sale_order_line_obj.browse(cr, uid, val, context=context)
...
#        raise osv.except_osv(_('Debug !'), _(str(pol_ids) + '----' + '' + '----' + ''))
        ...
        res['domain'] = {'sale_order_id': [('id','in',so_ids)]}

        if sale_order_id not in so_ids:
            res['value'].update({'sale_order_id': False, 'sale_order_line_id': False})
            res['warning'] = {'title': _('Warning'), 'message': _('The selected Sale Order is not belong to Product ' + str(product_product_obj.browse(cr, uid, product_id, context=context).name) + ' !')}
            return res

        if sale_order_line_id:
            sol3 = sale_order_line_obj.browse(cr, uid, sale_order_line_id, context=context)
            if sol3.order_id.id != sale_order_id:
                res['value'].update({'sale_order_line_id': False})
                res['warning'] = {'title': _('Warning'), 'message': _('The selected Sale Order Line is not belong to Sale Order ' + str(sale_order_obj.browse(cr, uid, sale_order_id, context=context).name) + ' !')}
            return res
#        raise osv.except_osv(_('Debug !'), _(str(pol_ids) + '----' + '' + '----' + ''))
...
#        res['domain'] = {'product_id': [('id','in',product_ids)]}
    ...
    def onchange_sale_order_line_id(self, cr, uid, ids , sale_order_line_id, purchase_order_line_id, context={}):
...
#        raise osv.except_osv(_('Debug !'), _(str(purchase_order_line_id) + '----' + str(pol3) + '----' + ''))
...
#        raise osv.except_osv(_('Debug !'), _(str(qty_sale_order) + '----' + str(qty_sale_order_received) + '----' + str(qty_purchase_order) + '----' + str(qty_sale_order2) + '----' + str(qty_can_allocated)))
    ...
    _columns = {
        ...
        'sale_order_id': fields.many2one('sale.order', 'Sale Order', ondelete='cascade', required=True),
        'sale_order_line_id': fields.many2one('sale.order.line', 'Sale Order Line', ondelete='cascade', required=True),
        'purchase_order_line_id': fields.many2one('purchase.order.line', 'Purchase Order Line', ondelete='cascade'),
