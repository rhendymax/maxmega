...
class param_sales_journal_by_voucher_report(osv.osv_memory):
    _name = 'param.sales.journal.by.voucher.report'
    _description = 'Param Sales Journal By Voucher Report'
    _columns = {
        ...
        'partner_ids' :fields.many2many('res.partner', 'report_receivable_journal_voucher_rel', 'report_id', 'partner_id', 'Customer', domain=[('customer','=',True)]),
    ...
    def create_vat(self, cr, uid, ids, context=None):
        ...
        datas['model'] = 'param.sales.journal.by.voucher.report'
        datas['form'] = self.read(cr, uid, ids)[0]
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'sales.journal.by.voucher.report_landscape',
            'datas': datas,
        }
    ...
    def check_report(self, cr, uid, ids, context=None):
        ...
        used_context = self.pool.get('param.sales.journal.by.voucher.report')._build_contexts(cr, uid, ids, data, None, context=context)

        return self.pool.get('param.sales.journal.by.voucher.report')._get_tplines(cr, uid, ids, used_context, None, context=context)
    ...
    def _get_tplines(self, cr, uid, ids,data, type, context):
        ...
        header += 'Vch No.;Date;Type;Customer Key; Customer Name;Cust PO No;Ccy;Exch Rate;Sales Person;Sales Zone;Pre Tax Amt;Sales Tax Amt;After Tax Amt;Pre Tax Home;Sales Tax Home;After Tax Home' + " \n"
        ...
        cr.execute(
                "SELECT l.id as inv_id, so.client_order_ref as cust_po " \
                "FROM account_invoice AS l LEFT JOIN account_invoice_line ail on ail.invoice_id = l.id  " \
                "LEFT JOIN stock_move sm on sm.id = ail.stock_move_id LEFT JOIN sale_order_line sol on sol.id = sm.sale_line_id " \
...
#                "and l.partner_id = " + str(s['id']) + " "\
                " order by l.number,l.date_invoice")
        ...
        if qry3:
            for t in qry3:
                ...
                + str(inv.partner_id and inv.partner_id.ref or '') + ';' \
                + str(inv.partner_id and inv.partner_id.name or '') + ';' \
                + str(t['cust_po'] or '') + ';' \
                + str(inv.currency_id and inv.currency_id.name or '') + ';' + str(inv.cur_rate or '') + ';' \
                + str(inv.user_id and inv.user_id.name or '') + ';' + str(inv.sales_zone_id and inv.sales_zone_id.name or '') + ';' \
            ...
            header += 'Total For Credit Note' + ';;;;;;;;;;;;' \
                   + ';' + str(total_pre_tax_home) + ';' + str(total_sale_tax_home) + ';' + str(total_after_tax_home) + ' \n \n'
        ...
        filename = 'Sales Journal By Voucher Report.csv'
        out = base64.encodestring(all_content_line)
        self.pool.get('param.sales.journal.by.voucher.report').write(cr, uid, ids,{'data':out, 'filename':filename})
        obj_model = self.pool.get('ir.model.data')
        model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','sales_journal_by_voucher_report_result_csv_view')])
        resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
        return {
                'name':'Sales Journal By Voucher Report',
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'param.sales.journal.by.voucher.report',
...
param_sales_journal_by_voucher_report()
