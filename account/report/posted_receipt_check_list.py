...
class posted_receipt_check_list(report_sxw.rml_parse):
    _name = 'posted.receipt.check.list'
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        self.customer_code_from = data['form']['customer_code_from'] or False
        self.customer_code_to = data['form']['customer_code_to'] or False
        self.partner_ids_selection = data['form']['partner_ids'] or False
        self.journal_ids_selection = data['form']['journal_ids'] or False
        return super(posted_receipt_check_list, self).set_context(objects, data, new_ids, report_type=report_type)

    def __init__(self, cr, uid, name, context=None):
        super(posted_receipt_check_list, self).__init__(cr, uid, name, context=context)
    ...
    def _get_lines(self):
        ...
        if self.filter_selection == 'cust_code':
            ...
            val_part = []
            val_part.append(('customer', '=', True))
            cust_from = self.customer_code_from
            if cust_from:
                self.cr.execute("select ref " \
                                "from res_partner "\
                                "where customer = True and " \
                                "ref ilike '" + str(cust_from) + "%' " \
                                "order by ref limit 1")
                qry = self.cr.dictfetchone()
                if qry:
                    val_part.append(('ref', '>=', qry['ref']))
            cust_to = self.customer_code_to
            if cust_to:
                self.cr.execute("select ref " \
                                "from res_partner "\
                                "where customer = True and " \
                                "ref ilike '" + str(cust_to) + "%' " \
        ...
        val_voucher.append(('type', '=', 'receipt'))
...
report_sxw.report_sxw('report.posted.receipt.check.list_landscape', 'account.voucher',
    'addons/max_custom_report/account/report/posted_receipt_check_list.rml', parser=posted_receipt_check_list, header="internal landscape")
