...
class maxmega_ar_payment_register_report(osv.osv_memory):
    _name = 'maxmega.ar.payment.register.report'
    _description = 'Customer Payment Register By Deposit Bank Report'
    ...
    def _get_tplines(self, cr, uid, ids,data, context):
		...
		header += 'Receipt No.,Receipt Date,Cheque Amt,Cheque Home Amt,Customer Reference,Customer Name,Currency,Exch Rate,Bank Draft,Charges,Charhes Home\n'
		customer_ids = form['customer_ids']
		if len(customer_ids) == 0:
		    customer_ids = pool.get('res.partner').search(cr, uid, [('customer','=',True)])
		if journal_id:
		    voucher_ids = pool.get('account.voucher').search(cr, uid, [('partner_id','in',customer_ids),('period_id','in',prds),('journal_id','=',journal_id)])
		else:
		    voucher_ids = pool.get('account.voucher').search(cr, uid, [('partner_id','in',customer_ids),('period_id','in',prds)])
		#voucher_ids = pool.get('account.voucher').search(cr, uid, [('period_id','in',prds),('journal_id','=',journal_id)],order='number')
		...
		model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','maxmega_ar_payment_register_result_data_view')])
		resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
		return {
		        'name':'Payment Register By Deposit Bank Report',
		        'view_type': 'form',
		        'view_mode': 'form',
		        'res_model': 'maxmega.ar.payment.register.report',
...
maxmega_ar_payment_register_report()
