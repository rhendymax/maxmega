...
class maxmega_ar_payment_reg_by_customer_report(osv.osv_memory):
    _name = 'maxmega.ar.payment.reg.by.customer.report'
    _description = 'Payment Register By Customer Report'
    _columns = {
    ...
    def _get_tplines(self, cr, uid, ids,data, context):
		...
		header += 'Receipt No.,Receipt Date,Cheque No.,Cheque Amt,Cheque Home Amt,Invoice No,Invoice Date,Currency,Currency Rate,Allocated Amt, Allocated Home, Charges, Charges Home\n'
		...
		csv_content = ''
		filename = 'Payment Register By Customer Report.csv'
		out = base64.encodestring(all_content_line)
		self.write(cr, uid, ids,{'data':out, 'filename':filename})
		obj_model = self.pool.get('ir.model.data')
		model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','maxmega_ar_payment_reg_by_customer_result_data_view')])
		resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
		return {
		        'name':'Payment Register By Customer Report',
		        'view_type': 'form',
		        'view_mode': 'form',
		        'res_model': 'maxmega.ar.payment.reg.by.customer.report',
...
maxmega_ar_payment_reg_by_customer_report()
