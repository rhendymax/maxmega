...
class maxmega_ap_payment_reg_by_supplier_report(osv.osv_memory):
    _name = 'maxmega.ap.payment.reg.by.supplier.report'
    _description = 'Payment Register By Supplier Report'
    ...
    def _get_tplines(self, cr, uid, ids,data, context):
		...
		#header += 'Paymant No.,Payment Date,Cheque No.,Invoice No,Invoice Date,Currency,Cheque Amount,Cheque Home Amount\n'
		header += 'Payment No.,Payment Date,Cheque No.,Cheque Amt,Cheque Home Amt,Invoice No,Invoice Date,Currency,Currency Rate,Allocated Amt, Allocated Home, Charges, Charges Home\n'
		...
		for sup in pool.get('res.partner').browse(cr, uid, supplier_ids):
			...
			if len(voucher_ids) > 0:
			    ...
			    for voucher in pool.get('account.voucher').browse(cr, uid, voucher_ids):
			        ...
			        header += str(voucher.number or '') + "," + str(voucher.date) + ","  + str(voucher.cheque_no or '') + "," + str(voucher.amount) +"," + str(voucher.amount) + "," + "" + "," + ""+ "," + str(cur_name) + "," + str(cur_rate) + "," + str(voucher.amount) + "," + str(allocated_home) + "," + str(bank_charge) + "," + str(bank_charge_home) + "\n"
			        for line in voucher.line_ids:
			            invoice_no = line.name or ''
			            invoice_date = line.date_original or ''
			            header += ",,,,," + str(invoice_no) + "," + str(invoice_date) + "\n"
		...
		filename = 'Payment Register By Supplier Report.csv'
		out = base64.encodestring(all_content_line)
		self.write(cr, uid, ids,{'data':out, 'filename':filename})
		obj_model = self.pool.get('ir.model.data')
		model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','maxmega_ap_payment_reg_by_supplier_result_data_view')])
		resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
		return {
		        'name':'Payment Register By Supplier Report',
		        'view_type': 'form',
		        'view_mode': 'form',
		        'res_model': 'maxmega.ap.payment.reg.by.supplier.report',
...
maxmega_ap_payment_reg_by_supplier_report()
