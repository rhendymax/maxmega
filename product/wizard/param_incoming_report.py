...
class param_incoming_report(osv.osv_memory):
    _name = 'param.incoming.report'
    _description = 'Param Incoming Report'
    _columns = {
        ...
        'product_ids' :fields.many2many('product.product', 'report_incoming_product_rel', 'report_id', 'product_id', 'Product', domain=[]),
        #Location Selection
        ...
        'sl_ids' :fields.many2many('stock.location', 'report_incoming_sl_rel', 'report_id', 'sl_id', 'Product', domain=[]),
    ...
    def create_vat(self, cr, uid, ids, context=None):
        ...
        datas['model'] = 'param.incoming.report'
        datas['form'] = self.read(cr, uid, ids)[0]
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'incoming.report_landscape',
            'datas': datas,
        }
...
#product_product
    ...
    def _get_tplines(self, cr, uid, ids,data, context):
        ...
        filename = 'Incoming Report.csv'
        out = base64.encodestring(all_content_line)
        self.write(cr, uid, ids,{'data':out, 'filename':filename})
        obj_model = self.pool.get('ir.model.data')
        model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','incoming_report_result_csv_view')])
        resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
        return {
                'name':'Incoming Report',
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'param.incoming.report',
