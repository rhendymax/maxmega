...
class change_cod(osv.osv_memory):
    _name = 'change.cod'
    _description = 'Change CRD'

    def do_reschedule(self, cr, uid, ids, context=None):
        sale_order_line_obj = self.pool.get('sale.order.line')
        for obj in self.browse(cr, uid, ids, context=context):
            sale_order_line_obj.write(cr, uid, obj.sale_order_line_id.id, {'customer_rescheduled_date': obj.customer_rescheduled_date}, context=context)
    ...
    def default_get(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        sale_order_line_obj = self.pool.get('sale.order.line')
        res = super(change_cod, self).default_get(cr, uid, fields, context=context)
    ...
    _columns = {
        ...
        'customer_rescheduled_date': fields.date('Customer Rescheduled Date', required=True, select=True),
        'reason_type': fields.selection([('etd','ETD'),('other', 'Other')], 'Reason Type', required=True,),
