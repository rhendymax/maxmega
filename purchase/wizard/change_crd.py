...
class change_crd(osv.osv_memory):
    _name = 'change.crd'
    _description = 'Change Customer Request Date'

    def default_get(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        purchase_order_line_obj = self.pool.get('purchase.order.line')
        res = super(change_crd, self).default_get(cr, uid, fields, context=context)
    ...
    def do_reschedule(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            purchase_order_line_obj.write(cr, uid, context.get(('active_ids'), []), {'cust_request_date': obj.change_date}, context=context)
        return {'type': 'ir.actions.act_window_close'}

    _columns = {
        'po_line_id': fields.many2one('purchase.order.line', 'Purchase Order Line', ondelete='cascade'),
        'change_date': fields.date('Customer Request Date', required=True, select=True),
